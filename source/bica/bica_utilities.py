"""
BicameralAGI Utility Functions Module
=====================================
This module provides a collection of utility functions used across the BicameralAGI system.
It offers a range of helpers for common tasks such as file operations, text processing,
random number generation, and environment variable management.

Key Features:
-------------
1. File Operations: Read files, load and save JSON.
2. Text Processing: Normalize text, calculate text similarity, extract keywords.
3. Data Manipulation: Merge dictionaries, clamp values, normalize weights.
4. Random Generation: Generate random floats.
5. Environment Management: Retrieve environment variables.
6. Formatting: Format lists as strings, generate timestamps.

Usage:
------
Import specific functions as needed:

    from bica_utilities import get_environment_variable, load_json_file
...
Functions:
----------
- get_environment_variable(var_name: str) -> str
- read_file(path: str) -> List[str]
- load_json_file(file_path: str) -> Dict[str, Any]
- save_json_file(data: Dict[str, Any], file_path: str) -> None
- generate_timestamp() -> str
- normalize_text(text: str) -> str
- calculate_similarity(text1: str, text2: str) -> float
- generate_random_float(min_value: float, max_value: float) -> float
- extract_keywords(text: str, max_keywords: int = 5) -> List[str]
- merge_dicts(dict1: Dict[str, Any], dict2: Dict[str, Any]) -> Dict[str, Any]
- clamp(value: float, min_value: float, max_value: float) -> float
- format_list_as_string(items: List[str], conjunction: str = "and") -> str
- normalize_weights(items: List[Dict[str, Any]], weight_key: str = "weight") -> List[Dict[str, Any]]
...
def main():
    ...
    similarity = calculate_similarity("Hello World", "Hello")
    print(f"Similarity: {similarity}")

    # Generate a random float
    random_float = generate_random_float(1.0, 2.0)
    ...
    with open(test_file, 'w') as f:
        f.write("Line 1\nLine 2\nLine 3\n")
    file_contents = read_file(test_file)
